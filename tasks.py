...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_demo_design_lifecycle_xxx
namespace = Collection("demo_design_lifecycle")
namespace.configure(
    {
        "demo_design_lifecycle": {
            "nautobot_ver": "3.0.0",
            "project_name": "demo_design_lifecycle",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.demo_design_lifecycle.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.demo_design_lifecycle.compose_http_timeout,
        "NAUTOBOT_VERSION": context.demo_design_lifecycle.nautobot_ver,
        "PYTHON_VER": context.demo_design_lifecycle.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.demo_design_lifecycle.project_name}",
        f'--project-directory "{context.demo_design_lifecycle.compose_dir}"',
    ]

    for compose_file in context.demo_design_lifecycle.compose_files:
        compose_file_path = os.path.join(context.demo_design_lifecycle.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.demo_design_lifecycle.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.demo_design_lifecycle.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.demo_design_lifecycle.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.demo_design_lifecycle.compose_dir, "creds.example.env"),
            os.path.join(context.demo_design_lifecycle.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if nautobot_ver is None:
        nautobot_ver = context.demo_design_lifecycle.nautobot_ver
    if python_ver is None:
        python_ver = context.demo_design_lifecycle.python_ver
    dockerfile_path = os.path.join(context.demo_design_lifecycle.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = (
                f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.demo_design_lifecycle.nautobot_ver}"
            )
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations demo_design_lifecycle"
...
def pylint(context):
    ...
    command = f"{base_pylint_command} demo_design_lifecycle"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("demo_design_lifecycle") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " demo_design_lifecycle.migrations"
        )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 demo_design_lifecycle/templates/"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.demo_design_lifecycle.local):
